...
def xBins(X, bin_type):
    ...
    if bin_type == 'amd':
        ...
        amd_bins = [0, 1, 50, 150, 400, 600, 1200, 2500, 4000]# Change 2000 bsk to 2500 later    
    ...
    #     clusterer = MiniBatchKMeans(n_clusters=16, random_state=10) # changed the number of clusters from 8 to 16
    ...
    # if bin_type == 'daytype':
    #     print("Inside bin_X function pre-binning by daytype")
    #     Xd =  X.reset_index()

    #     work_week = ['Monday','Tuesday','Wednesday','Thursday']

    #     Xd['weekday'] = Xd.date.dt.day_name()

    #     Xd['daytype'] = Xd.weekday.where(~Xd.weekday.isin(work_week), 'Weekday')
    #     Xd['date'] = Xd['date'].dt.strftime('%Y-%m-%d')

    #     Xbin_dict = dict()
    #     Xbin_dict = Xd.groupby('daytype').date.apply(list).to_dict()
